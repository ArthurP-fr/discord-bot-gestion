...
} from "../../legacy/validators/presence.js";
import type { PresenceRepository } from "../../legacy/modules/presence/index.js";
...
const parseStoredTexts = (rawTexts: string | null, fallbackText: string): string[] => {
  if (typeof rawTexts === "string" && rawTexts.trim().length > 0) {
    try {
      ...
      // Fall back to legacy single text when malformed JSON is encountered.
    }
  }
...
export class TenantPresenceStore implements PresenceRepository {
  public constructor(
  ...
  public async getByBotId(botId: string): Promise<PresenceState> {
    const result = await this.pool.query<PresenceRow>(
      `
      SELECT status, activity_type, activity_text, activity_texts, rotation_interval_seconds
      FROM bot_presence_states
